...
function setStatus(id, status, data) {
  ...
  switch (status) {
    case 'started':
      ...
      log_data = { profile_id: id, status, timeout: data };
      break;
    case 'finished':
      message = `Finished profiling id ${id}`;
      log_data = { profile_id: id, status };
      break;
    case 'failed':
      message = `Failed to profile id ${id}, Error: ${data}`;
      log_data = { profile_id: id, status, error_message: data };
      break;
    default:
      return;
  }
  ...
  eventEmitter.emit('status_change', { id, status, data });
